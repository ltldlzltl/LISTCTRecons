...
)
{
	...
	for (int ia = a_min + ia_begin; ia < a_max; ia += ia_step)
	{
		...
		float delta_p = theta*ctg_c.dsd_() / ctg_c.ds_();
		...
		float qhat = (zc - z_ray)*ctg_c.dsd_()*ctg_c.z_dir() / (lhat*ctg_c.dt_());
		float qhat2 = (zc - z_ray2)*ctg_c.dsd_()*ctg_c.z_dir() / (lhat2*ctg_c.dt_());
		...
		{
			ww = abs(qhat2) / (abs(qhat) + abs(qhat2));
			ww2 = abs(qhat) / (abs(qhat) + abs(qhat2));
		}
